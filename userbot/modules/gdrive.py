...
async def gdrive_upload_function(dryb):
    ...
    if "|" in input_str:
        ...
        if os.path.exists(input_str):
            ...
            await dryb.edit(
                "Trovato `{}` nel server, Ora carico su drive..."
                .format(input_str))
        else:
            await dryb.edit(
                "File non trovato. Dammi un percorso valido!")
            return False
        ...
        try:
            ...
            downloaded_file_name = await dryb.client.download_media(
                ...
                ).create_task(progress(d, t, dryb, c_time, "Scaricando...")))
        except Exception as e:
    ...
    if required_file_name:
        ...
        try:
            g_drive_link = await upload_file(http, required_file_name,
                                             ...
                                             parent_id)
            await dryb.edit(
                f"File:`{required_file_name}`\nè stato caricato correttamente a [Google Drive]({g_drive_link})!"
            )
        except Exception as e:
            await dryb.edit(
                f"Errore nel caricamento su Drive\nCodice errore:\n`{e}`")
...
async def upload_dir_to_gdrive(event):
    ...
    if os.path.isdir(input_str):
        ...
        await event.edit(f"Ecco il tuo link Google Drive [link]({dir_link})")
    else:
        await event.edit(f"La cartella {input_str} sembra che non esista")
...
async def gdrive_search_list(event):
    ...
    http = authorize(G_DRIVE_TOKEN_FILE, storage)
    # Authorize, get file parameters, upload file and print out result URL for download
    await event.edit(f"Cercando per {input_str} nel tuo Google Drive...")
...
async def download(set):
    ...
    await set.edit("Caricamento...")
    input_str = set.pattern_match.group(1)
    if input_str:
        parent_id = input_str
        await set.edit(
            "Id cartella impostato correttamente. I prossimi saranno caricati su {parent_id} fino a `.gdriveclear`"
        )
        await set.delete()
    else:
        await set.edit(
            "Usa `.gdrivesp <link to GDrive Folder>` per impostare la cartella dove caricare i file."
        )
...
async def download(gclr):
    ...
    await gclr.reply("Caricamento...")
    parent_id = GDRIVE_FOLDER_ID
    await gclr.edit("ID eliminato correttamente.")
...
async def show_current_gdrove_folder(event):
    if parent_id:
        ...
        await event.edit(
            f"Il mio userbot sta caricando i file [qui]({folder_link})")
    else:
        await event.edit(
            f"Il mio userbot sta caricando i file nella cartella di root.\
            \nTrova i file caricati [qui](https://drive.google.com/drive/my-drive)"
        )
...
async def create_token_file(token_file, event):
    ...
    async with event.client.conversation(BOTLOG_CHATID) as conv:
        await conv.send_message(
            f"Vai su questo link dal browser: {authorize_url} e rispondi con il codice"
        )
        ...
        if status:
            ...
            progress_str = "[{0}{1}] {2}%".format(
                ...
                "".join(["░"
                         for i in range(10 - math.floor(percentage / 10))]),
                round(percentage, 2))
            current_message = f"Caricando su Google Drive\nNome del file: {file_name}\n{progress_str}"
...
async def create_directory(http, directory_name, parent_id):
    ...
    drive_service.permissions().insert(fileId=file_id,
                                       body=permissions).execute()
    LOGS.info("Creata cartella Google Drive:\nNome: {}\nID: {} ".format(
...
CMD_HELP.update({
    ...
    \nUtilizzo: Carica il file in risposta, URL o percorso file su Gdrive.\
    \n\n.gsetf <GDrive Folder URL>\
    \nUtilizzo: Imposta la cartella dove saranno caricati i file.\
    \n\n.gsetclear\
    \nUsage: Reimposta la cartella dove i file vengono caricati.\
    \n\n.gfolder\
    \nUtilizzo: Mostra il percorso dove i file vengono caricati.\
    \n\n.list <query>\
    \nUtilizzo: Cerca file e cartelle nel tuo Gdrive.\
    \n\n.ggd <path_to_folder_in_server>\
    \nUtilizzo: Carica tutti i file nella cartella del server su Gdrive."
})
